...
from big_data_getl.load import load_rdd, load_json, load_xml
from tests.data.load.example_schema import (
    create_json_schema,
    create_valid_schema
)
...
def test_load_xml_with_invalid_params(spark_session, schema, rowtag):
    """load_xml should retun success for an invalid schema or rowtag."""
...
def test_load_rdd_passes_correct_args_to_spark(paths, val_paths):
...
def test_load_rdd_successful(spark_session, file_paths):
    ...
    assert 'MapPartitionsRDD' in str(load_rdd(spark_session, file_paths))
...
def test_load_exception(load_function, function_params, error_msg):
    ...
    assert "'Builder' object has no attribute '{}'".format(error_msg) in str(
        excinfo)
